...
- 雙 host（Codex／Claude）12 valid runs 的正式 pilot **未完成**：原 Linux 研究環境因 host 未登入而阻塞，原報告記錄已準備 runbook、trace collector／scorer，以及模擬 trace 檢查。2026-09-18 06:24 UTC 在接手機器 macOS 重做預檢，Codex 0.155.0 與 Claude Code 2.1.275 均回報已登入；但 `/workspace/sdd-outcome-research/` 不存在，Git 也未包含凍結原型、runner、prompts 與 adapters，無法校驗或啟動原實驗。本次為 live 執行前的 setup failure，0 次 live attempts、0／12 valid runs，不是候選有效失敗；先前 7 項測試亦未在本機重跑。另 CLI doctor 回報本機缺少 approval manifest，需先恢復 machine baseline 才能更新完成狀態。詳見 `docs/reports/outcome-driven-workflow-design/resume-preflight-2026-09-18.md`。目前 **不能** 主張候選降低中斷或成本，也 **不能** 建議直接採用為正式產品流程。
