...
> 版本 v0.2.2 ｜ [English](./README.en.md)
...
| 放棄 | `放棄`／`取消提案` → `確認放棄 <短名稱>` | 先執行唯讀 preflight：回報進度、警告工作區程式碼不會復原、把內容 hash 列在回報中。收到一字不差的 `確認放棄 <短名稱>` 且內容未變，才標記 `abandoned` 歸檔並更新索引；單獨說「取消」只會先詢問目標 |
