...
> 版本 v1.4.0 ｜ [English](./README.en.md)
...
- **工程品質查證**：開始 task 前整理最小必要 context；版本相依決策查對官方文件；完成前只套用專案明確宣告且相關的品質標準。
...
跨檔案或跨模組工作會依相依順序安排 task，並優先切成每條完成後仍可運作、可獨立驗證的垂直成果；不以固定檔案數限制拆分，也不因此新增另一份 planning artifact。

實作時，版本相依的 framework、library、SDK 或工具決策會先核對專案實際版本與官方文件；`complete-task` 前只採用 project instructions、contributor guidance 或 CI 文件明確宣告且與變更相關的 Definition of Done。沒有宣告不會自行發明檢查，衝突宣告會先停下回報。
...
這個 minor release 新增有界的高風險 proposal intake、approval-relevant baseline 與 authority-split 檢查，並提供需明確確認、可重試與可安全 restore 的 legacy proposal／archive reconstruction。現有有效 proposal 不需轉換；proposal schema v1/v2 與 JSON output version 均不變。
