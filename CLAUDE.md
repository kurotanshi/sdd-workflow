...
Automation covers parser fixtures, CLI behavior, package/runtime checks, docs consistency, and other hermetic rules. Static checks plus versioned, isolated non-interactive Codex and Claude runs are the complete workflow acceptance evidence; no human host session is required. Eval spec v2 uses an explicit affected-scenario selection with one valid adherent run per Agent/scenario cell, exact candidate/model/spec identity, and a handoff rationale. Aggregate adherence is diagnostic-only; any failed selected cell, exhausted invalid run, identity mismatch, or Critical Violation blocks release. Full three-run coverage is a separate explicit benchmark. See CONTRIBUTING.md and `docs/agent-eval.md` for the complete contract.
