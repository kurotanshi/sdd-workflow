...
grep -Fq '> 版本 v1.1.1' "$readme_zh" || {
  echo "trigger-contract: README.md must report v1.1.1" >&2
  exit 1
}

grep -Fq '> Version v1.1.1' "$readme_en" || {
  echo "trigger-contract: README.en.md must report v1.1.1" >&2
  exit 1
}
