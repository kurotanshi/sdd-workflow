...
| task scanner | 規則文字存在，可用 fixture 模擬掃描結果 | 縮排／巢狀／`- [X]`／`- [xx]`／`- []`／`1. [ ]` 等格式錯誤，以及任務區內混入其他清單項（含以連結開頭的 `- [參考](…)`）時，實作（approval gate 前）、preflight、修訂與歸檔停止並報行號 |
| 放棄 preflight | 規則文字存在 | 「放棄」／「取消提案」只回報進度、警告程式碼不復原、把兩份檔案的 hash 列在回報中後停止；狀態、目錄與 INDEX 均未變 |
| 確認放棄 | 規則文字存在 | 一字不差的「確認放棄 <短名稱>」且重算 hash 未變才執行；名稱不符、缺 snapshot（含新 session）或 hash 變更時重新 preflight |
| 「取消」語意 | 規則文字存在 | 裸「取消」或指涉不明時先詢問要復原程式碼還是放棄提案，不直接執行任一種；明確指向程式碼的取消當一般復原請求處理——先確認範圍，絕不觸碰提案 |
