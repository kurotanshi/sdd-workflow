...
| task scanner | 規則文字存在，可用 fixture 模擬掃描結果 | 縮排／巢狀／`- [X]`／`- [xx]`／`- []`／`1. [ ]` 等格式錯誤時，實作（approval gate 前）、preflight、修訂與歸檔停止並報行號 |
| 放棄 preflight | 規則文字存在 | 「放棄」／「取消提案」只回報進度、警告程式碼不復原、記錄 hash 後停止；狀態、目錄與 INDEX 均未變 |
| 確認放棄 | 規則文字存在 | 一字不差的「確認放棄 <短名稱>」且重算 hash 未變才執行；名稱不符、缺 snapshot（含新 session）或 hash 變更時重新 preflight |
| 裸「取消」 | 規則文字存在 | 一律先詢問要復原程式碼還是放棄提案，不直接執行任一種 |
